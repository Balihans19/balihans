...
// Add this flag to control submissions at the backend level
const SUBMISSIONS_ENABLED = process.env.SUBMISSIONS_ENABLED === 'true';// Set to false to disable all submissions
...
const submitLimit = rateLimit({
  ...
  message: {
    message: "Too many resume submissions from this IP, please try again later.",
  },
  standardHeaders: true,
  legacyHeaders: false,
});

router.post("/email", submitLimit, async (req, res) => {
  // Check if submissions are enabled at the backend level
  if (!SUBMISSIONS_ENABLED) {
    return res.status(503).json({
      message: "Sorry, we have been receiving too many resumes lately. Allow us to process them and we will be accepting resumes again",
...
// Add an endpoint to check if submissions are enabled
router.get("/status", (req, res) => {
  res.json({
    submissionsEnabled: SUBMISSIONS_ENABLED,
    message: SUBMISSIONS_ENABLED 
      ? "Resume submissions are currently open." 
      : "Sorry, we have been receiving too many resumes lately. Allow us to process them and we will be accepting resumes again"
  });
});
