...
const CaseStudySlides = () => {
  ...
  // Fetch random slides from the backend
  useEffect(() => {
    const fetchSlides = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/casestudyslidespage/random`); 
            ...
            >
              {slides.map((slide, index) => (
                <div
                  ...
                  } flex-shrink-0 px-4 md:px-10 flex flex-col items-center cursor-pointer`}
