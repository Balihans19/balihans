  ...
  useEffect(() => {
    const handleScroll = () => {
      if (contentRef.current) {
        ...
        // Rest of the existing scroll handling logic
        ...
        if (scrollTop === 0) {
          ...
          for (let i = 0; i < sections.length; i++) {
            ...
            if (
              ...
              (sectionMiddle >= scrollTop && sectionMiddle <= scrollBottom) ||
              (i < sections.length - 1 && sectionBottom >= scrollTop && sections[i + 1].offsetTop > scrollBottom)
            ) {
