...
const CaseStudySolutions = ({ solutionsData = [], title }) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (contentRef.current) {
        ...
        // Handle edge case: if scrolled to the very bottom
          ...
          for (let i = 0; i < sections.length; i++) {
            ...
            // Check if section is fully visible or if its middle point is visible
            if (
              (sectionTop <= scrollTop && sectionBottom >= scrollBottom) || // Fully visible
              (sectionMiddle >= scrollTop && sectionMiddle <= scrollBottom) || // Middle point visible
  ...
  const scrollContent = (direction) => {
    if (contentRef.current) {
      const scrollAmount = contentRef.current.clientHeight / 5;
