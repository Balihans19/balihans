...
const CaseStudySolutions = ({ solutionsData = [], title }) => {
  ...
  >
    {solutionsData.map((solution) => (
      <div key={solution.id} className="flex flex-col gap-8 min-h-[550px]">
        {solution.contents.map((content, idx) => (
          <div key={idx} className="mb-1">
              ...
              <img
                src={content.image}
                alt="Solution Visual"
                className="rounded-lg shadow-lg w-15 h-15"
              />
            )}
            {content.description && (
            ...
            {content.primaryHeading && (
              <h3 className="text-2xl font-bold">{content.primaryHeading}</h3>
            )}
